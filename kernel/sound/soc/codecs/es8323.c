...
#define mute_delay  800
static bool hp_irq_flag = 0;
//extern int hdmi_state;
//SPK HP vol control
//#define es8323_DEF_VOL	0x1b
#ifndef es8323_DEF_VOL 
//#define es8323_DEF_VOL	        0x19   //1.4W  1.2-1.5   14 15.6
#define es8323_DEF_VOL	        0x1c       //1.99W  1.8-2.2  18.5 21.5 24 27 32
//#define es8323_DEF_VOL	        0x1d       //2.17W  1.8-2.2  18.5 21.5 24 27 32
//#define es8323_DEF_VOL	        0x21//0x23       //2.80W  2.8-3.5    43 55
#endif

static int es8323_set_bias_level(struct snd_soc_codec *codec,
				 enum snd_soc_bias_level level);
static int shutdown_hp(int flag);
/*
...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	struct es8323_priv *es8323 = es8323_private;
        ...
        printk("\n\n\n\n\n--------es8323 hp_det_irq_handler------\n\n\n");
        printk("--------es8323 hp_det_irq_handler------spk_gpio_level=%d\n",es8323->spk_gpio_level);
        printk("--------es8323 hp_det_irq_handler------gpio_get_value(es8323->hp_det_gpio)=%d\n",gpio_get_value(es8323->hp_det_gpio));
		if(gpio_get_value(es8323->hp_det_gpio) == 1)
		    shutdown_hp(1);
	/* for steady */
    msleep(300);
	...
	if(es8323->hp_det_level == gpio_get_value(es8323->hp_det_gpio)){
		printk("------es8323 insert hp------\n");
		switch_set_state(&es8323->sdev, 1);
		
	}else{
		printk("------es8323 unsert hp-------\n");
		switch_set_state(&es8323->sdev, 0);
	}		
...
static int es8323_mute(struct snd_soc_dai *dai, int mute)
{
	...
	else
	{
		...
		snd_soc_write(codec, 0x30,es8323->spk_volume);
		snd_soc_write(codec, 0x31,es8323->spk_volume);
	
		msleep(130);
		printk("qwang    hp_irq_flag = %d\n", hp_irq_flag);
		if(hp_irq_flag == 1)
			es8323_set_gpio(ES8323_CODEC_SET_SPK,!es8323->spk_gpio_level);
		else
		{
...
static int es8323_probe(struct snd_soc_codec *codec)
{
	...
	int ret = 0;
   	...
   	printk("----MAC es8323_probe 20181230 %s\n", __func__);
	...
	es8323_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
        ...
        /*  add spk ctl pin  */	
	msleep(1000);
        printk("es8323_iic_init----\n"); 
	if(es8323->hp_det_level == gpio_get_value(es8323->hp_det_gpio)){
			printk("---es8323_probe   hp_det_level = 1,insert hp\n");
			switch_set_state(&es8323->sdev, 1);
			es8323_set_gpio(ES8323_CODEC_SET_SPK,es8323->spk_gpio_level);
			es8323_set_gpio(ES8323_CODEC_SET_HP,!es8323->hp_gpio_level);
		}else{
			printk("---es8323_probe   hp_det_level = 0,uninsert hp\n");
			switch_set_state(&es8323->sdev, 0);
			es8323_set_gpio(ES8323_CODEC_SET_SPK,es8323->spk_gpio_level);
			es8323_set_gpio(ES8323_CODEC_SET_HP,!es8323->hp_gpio_level);
		}	
...
static int shutdown_hp(int flag){
	struct es8323_priv *es8323 = es8323_private;
	printk("enter shutdown_hp\n");
	if(flag)   ///插入耳机瞬间音量过大
	{
		snd_soc_write(es8323_codec, 0x30, 0x00);
        snd_soc_write(es8323_codec, 0x31, 0x00);
	}
	else
	{
		snd_soc_write(es8323_codec, 0x30, es8323->spk_volume);
        snd_soc_write(es8323_codec, 0x31, es8323->spk_volume);
	}
}
EXPORT_SYMBOL(shutdown_hp);
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (es8323->spk_ctl_gpio < 0) {
	    ...
	    es8323->spk_gpio_level = (flags & OF_GPIO_ACTIVE_LOW)? 1:0;
	...
	if (es8323->hp_det_gpio < 0) {
		...
		if (ret != 0) {
			printk(&i2c->dev, "Failed to request hp_det_gpio\n");
			return ret;
		}
		...
		if (hp_irq) {
			...
			if(ret == 0){
				...
				printk(&i2c->dev, "request_irq failed: %d\n", ret);
				return ret;
			}
		}
	}

	ret = of_property_read_u32(i2c->dev.of_node, "spk_volume",
				   &es8323->spk_volume);
	if (ret < 0) {
		DBG(KERN_ERR"%s() Can not read property spk_volume, default 25\n",
			__func__);
		es8323->spk_volume = 0x1c;
	}

#if 1 //headset  hyman
	es8323->sdev.name = "h2w";
	es8323->sdev.print_name = h2w_print_name;
	ret = switch_dev_register(&es8323->sdev);
	if (ret < 0){
		printk("es8323 switch_dev_register fail\n");
		return ret;
	}
//	switch_set_state(&es8323->sdev, 0);
#endif	
