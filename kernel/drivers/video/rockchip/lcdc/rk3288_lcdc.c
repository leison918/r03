...
{	
	...
	if (likely(lcdc_dev->clk_on)) {
		...
		val = v_DSP_HOLD_VALID_INTR_CLR(1) | v_FS_INTR_CLR(1) |
			v_LINE_FLAG_INTR_CLR(1) | v_BUS_ERROR_INTR_CLR(1);
		lcdc_msk_reg(lcdc_dev, INTR_CTRL0, mask, val);
		...
		val = v_WIN0_EMPTY_INTR_CLR(1) | v_WIN1_EMPTY_INTR_CLR(1) |
			v_WIN2_EMPTY_INTR_CLR(1) | v_WIN3_EMPTY_INTR_CLR(1) |
			v_HWC_EMPTY_INTR_CLR(1) | v_POST_BUF_EMPTY_INTR_CLR(1) |
...
static int rk3288_lcdc_early_resume(struct rk_lcdc_driver *dev_drv)
{
	...
	if (dev_drv->trsm_ops && dev_drv->trsm_ops->enable && (atomic_read(&sys_poweroff_flag)==INIT_PM_FLAG))
		dev_drv->trsm_ops->enable();
...
static irqreturn_t rk3288_lcdc_isr(int irq, void *dev_id)
{
	...
	if(intr0_reg & m_FS_INTR_STS){
		...
		intr1_reg = lcdc_readl(lcdc_dev, INTR_CTRL1);
                ...
                lcdc_cfg_done(lcdc_dev);
                printk("%s>>>>>>other lcdc isr is running, intr0_reg:0x%x,intr1_reg:0x%x\n",__func__, intr0_reg,intr1_reg);
	}
