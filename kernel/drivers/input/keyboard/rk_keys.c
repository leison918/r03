...
#define ADC_DEBOUNCE_JIFFIES        (5 / (MSEC_PER_SEC / HZ))      /* 10ms */
...
struct rk_keys_drvdata {
	...
	struct delayed_work adc_rechk_work;
...
void rk_send_power_key(int state)
{
	if (!sinput_dev)
		return;
	if(atomic_read(&sys_poweroff_flag)==INIT_PM_FLAG){
	if (state) {
...
static void keys_timer(unsigned long _data)
{
	...
	if (button->state != state) {
		...
		input_event(input, EV_KEY, button->code, button->state);
		input_sync(input);
        if(atomic_read(&sys_poweroff_flag)==ENTER_PM_FLAG \
                && button->code==KEY_POWER){
           atomic_set(&sys_poweroff_flag,EXIT_PM_FLAG);
           printk("%s>>>>>>>>power key is press,so reboot machine\n",__func__);
        }
	}
...
static void adc_key_rechk(struct work_struct *work)
{
        struct rk_keys_drvdata *ddata;
        int i, result = -1;

        ddata = container_of(work, struct rk_keys_drvdata, adc_rechk_work.work);
        if (!ddata->in_suspend) {
                result = rk_key_adc_iio_read(ddata);
                printk("%s>>>>>result=%d\n",__func__,result);
                for (i = 0; i < ddata->nbuttons; i++) {
                    struct rk_keys_button *button = &ddata->button[i];
                    if(!button->rechk_flag)
                       continue;

                    button->rechk_flag=0;

                    if (result < button->adc_value + DRIFT_DEFAULT_ADVALUE &&
                            result > button->adc_value - DRIFT_DEFAULT_ADVALUE){
                        button->adc_state = 1;
                    }else{
                        printk("%s>>>>>>>>del adc key button timer\n",__func__);
                        button->adc_state = 0;
                        del_timer_sync(&button->timer); 
                    }
                }
        }
}
	...
	if (!ddata->in_suspend) {
		...
		for (i = 0; i < ddata->nbuttons; i++) {
			...
			if (button->state != button->adc_state) {
                if(button->adc_state && (result >= 0) && !button->state){
                    //press key
                    button->rechk_flag=1;
                    schedule_delayed_work(&ddata->adc_rechk_work, ADC_DEBOUNCE_JIFFIES);
                }
...
static int keys_probe(struct platform_device *pdev)
{
	...
	if (ddata->chan) {
		...
		schedule_delayed_work(&ddata->adc_poll_work,
				      ADC_SAMPLE_JIFFIES);
		INIT_DELAYED_WORK(&ddata->adc_rechk_work, adc_key_rechk);
	}
...
static int keys_remove(struct platform_device *pdev)
{
	...
	for (i = 0; i < ddata->nbuttons; i++)
		del_timer_sync(&ddata->button[i].timer);
	if (ddata->chan) {
		cancel_delayed_work_sync(&ddata->adc_rechk_work);
