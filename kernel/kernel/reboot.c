...
atomic_t sys_poweroff_flag;
extern void power_off_backlight(void);
static void kernel_enter_pm_if_shutdown(void)
{
    int status;

    atomic_set(&sys_poweroff_flag,ENTER_PM_FLAG);
    power_off_backlight();
    while(true){
       if(atomic_read(&sys_poweroff_flag)==EXIT_PM_FLAG)
          break;
       status=pm_suspend(PM_SUSPEND_MEM);
       if(status<0){
          printk("%s>>>>>pm_suspend failed,err=%d\n",__func__,status);
       }
       if(atomic_read(&sys_poweroff_flag)==EXIT_PM_FLAG)
          break;
       else
    ...
    printk("%s>>>>>ir remote power or rtc alarm is coming,so shutdown machine\n",__func__);
    kernel_restart(NULL);
    do_exit(0);
    return;
}
void kernel_pm_suspend_init(void)
{
   atomic_set(&sys_poweroff_flag,INIT_PM_FLAG);
}
EXPORT_SYMBOL_GPL(kernel_pm_suspend_init);
